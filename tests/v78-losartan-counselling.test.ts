...
describe("v78 losartan multilingual counselling §9", () => {
  it("publishes all five SA launch languages for losartan", () => {
    const langs = listCounsellingLangs("mol-losartan");
    assert.deepEqual(langs.sort(), ["af", "en", "st", "xh", "zu"]);
    const cov = counsellingCoverage("mol-losartan");
    assert.equal(cov.length, 5);
    assert.ok(cov.every((c) => c.lineCount === 4));
    assert.ok(listMoleculesWithPublishedCounselling().includes("mol-losartan"));
  });

  it("teaches ARB pregnancy and angioedema cues without inventing doses or BP targets", () => {
    for (const lang of ["en", "zu", "af", "st", "xh"] as const) {
      const script = getCounsellingScript("mol-losartan", lang);
      assert.ok(script);
    ...
    const en = getCounsellingScript("mol-losartan", "en")!;
    assert.match(en.lines.join(" "), /ARB|pregnan/i);
