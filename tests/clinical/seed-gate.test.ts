...
describe("seed S0 gate", () => {
  ...
  it("resolves Tegretol → carbamazepine from neurology seed", () => {
    const hits = resolveSearch("Tegretol", molecules, products);
    assert.equal(hits[0]?.moleculeSlug, "carbamazepine");
  });

  it("resolves Epilim → valproate from neurology seed", () => {
    const hits = resolveSearch("Epilim", molecules, products);
    assert.equal(hits[0]?.moleculeSlug, "valproate");
  });

  it("publishes educational carbamazepine↔ethinylestradiol interaction without inventing a dose", () => {
    const ix = seeds
      .flatMap((s) => s.interactions ?? [])
      .find((i) => i.id === "ix-carbamazepine-ethinylestradiol");
    assert.ok(ix);
    assert.equal(ix.publishState, "published");
    assert.equal(ix.severity, "major");
